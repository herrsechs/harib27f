	...
	*((int *) 0x0fe8) = (int) nihongo;
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (256 <= i && i <= 511) { /* ƒL[ƒ{[ƒhƒf[ƒ^ */
				...
				if (i == 256 + 0x3b && key_shift != 0 && key_win != 0) {	/* Shift+F1 */
					...
					if (task != 0 && task->tss.ss0 != 0) {
						...
						io_sti();
						task_run(task, -1, 0);	/* I—¹ˆ—‚ðŠmŽÀ‚É‚â‚ç‚¹‚é‚½‚ß‚ÉAQ‚Ä‚¢‚½‚ç‹N‚±‚· */
					}
				}
				if (i == 256 + 0x3c && key_shift != 0) {	/* Shift+F2 */
					/* V‚µ‚­ì‚Á‚½ƒRƒ“ƒ\[ƒ‹‚ð“ü—Í‘I‘ðó‘Ô‚É‚·‚éi‚»‚Ì‚Ù‚¤‚ªeØ‚¾‚æ‚ËHj */
					if (key_win != 0) {
						keywin_off(key_win);
					}
					key_win = open_console(shtctl, memtotal);
									...
									if (sht->buf[y * sht->bxsize + x] != sht->col_inv) {
										...
										if (sht->bxsize - 21 <= x && x < sht->bxsize - 5 && 5 <= y && y < 19) {
											...
											if ((sht->flags & 0x10) != 0) {		/* ƒAƒvƒŠ‚ªì‚Á‚½ƒEƒBƒ“ƒhƒE‚©H */
												...
												task = sht->task;
												sheet_updown(sht, -1); /* ‚Æ‚è‚ ‚¦‚¸”ñ•\Ž¦‚É‚µ‚Ä‚¨‚­ */
												keywin_off(key_win);
												key_win = shtctl->sheets[shtctl->top - 1];
												keywin_on(key_win);
												io_cli();
                                            ...
                                            if(  ql_clicked != -1){
                                                ...
                                                //run_app(fat, "invader");
                                                fifo32_put(&keycmd, 0x15 + 256);
                                                cons_putstr0(task->cons, fifo32_get(&keycmd));
                                                //open_console(shtctl, memman);
                                            }
			...
			} else if (2024 <= i && i <= 2279) {	/* ƒRƒ“ƒ\[ƒ‹‚¾‚¯‚ð•Â‚¶‚é */
